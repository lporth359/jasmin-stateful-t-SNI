...
let observation_empty : observation = None
  ...
  let sis obs = 
    (*print_endline "Debug begin: sis";
    print_endline (Printf.sprintf "Input observation: %s" (print_obs_s obs) );*)
    let obs' = eliminate_masking obs in

    (*print_endline (Printf.sprintf "Dep = {%s}" (print_dep_s (collect_shares obs') ""));
    print_endline "Debug end: sis";*)

...
let global_allocation return_addresses (funcs: ('info, 'asm) func list) :
  ...
  let preprocess f =
  let f1 = f |> fill_in_missing_names in


  let f2 = Ssa.split_live_ranges false f1 in


  Hf.add liveness_table f2.f_name (Liveness.live_fd true f2);

  (* rest of preprocess should use f2 *)
  let ra = Hf.find return_addresses f2.f_name in
  let written =
    let written, cg = written_vars_fc f2 in
    let written =
      match f2.f_cc with
      | (Export | Internal) -> written
      | Subroutine -> Sv.union (vars_retaddr ra) written
    in
    let killed_by_calls =
      Mf.fold (fun fn _locs acc -> Sv.union (killed fn) acc)
        cg Sv.empty
    in
    let killed_by_syscalls = if has_syscall f2.f_body then Arch.syscall_kill else Sv.empty in
    Sv.union (Sv.union written killed_by_calls) killed_by_syscalls
  in
  Hf.add killed_map f2.f_name written;
  f2
  in

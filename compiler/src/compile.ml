...
let compile (type reg regx xreg rflag cond asm_op extra_op)
  ...
  let module SecTrafo = Securitytransformation.Securitytransformation (Arch) in
  ...
  let securitytransformation (sprog) =
    SecTrafo.security_transform_sprog Arch.asmOp sprog
  in 

  let global_regalloc fds =
  
  
  let (fds, smap, rmap, omap) = Transformannot_util.create_shareMap fds in
    ...
    let fds = Transformannot_util.rekey_fds fds subst in 
    fds
  in
  ...
  let cparams =
    {
      ...
      Compiler.pp_sr;
      Compiler.security_transform_sprog = securitytransformation;
    }
  in

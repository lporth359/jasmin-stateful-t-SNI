...
let find_glob ii xi gd ws w =
  let test = fun gv0 ->
    if (&&) (convertible (Coq_aword ws) (Var.vtype (fst gv0)))
         (check_data (snd gv0) ws w)
    then Some (fst gv0)
    else None
  in
...
let add_glob fresh_id ii x gd ws w =
  let test = fun gv0 ->
    (&&) (convertible (Coq_aword ws) (Var.vtype (fst gv0)))
      (check_data (snd gv0) ws w)
  in
  if has test gd
  then Ok gd
       ...
       if has (fun g' ->
            eq_op Var.coq_MvMake_var__canonical__eqtype_Equality
              (fst (Obj.magic g')) (Obj.magic gx)) gd
